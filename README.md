...
- **Autonomous Navigation & Goal Execution**: Interactive click-to-navigate canvas with goal dispatching (`/goal_pose` / `NavigateToPose`), "Return Home", and immediate goal cancellation via rosbridge action service calls with zero-velocity safety stopping.
- **Ultra-Low Latency Video Streaming**: Primary WebRTC streaming via WHEP (MediaMTX on port 8889, `<200ms` latency) with automatic fallback to HTTP MJPEG (`web_video_server` on port 8080) and platform-aware defaults.
